...
class InvoicePrinter extends FPDF
{
    ...
    public function Header()
    {
        ...
        if (isset($this->title) and !empty($this->title)) {
            $this->Cell(0, 5, Encoding::fixUTF8($this->title), 0, 1, 'R');
        }
        ...
        if (!empty($this->reference)) {
            ...
            $this->Cell(
                32,
                $lineheight,
                    Encoding::fixUTF8($this->lang['number']),
                0,
                0,
                'L'
            );
        ...
        $this->SetFont($this->font, 'B', 9);
        $this->SetTextColor($this->color[0], $this->color[1], $this->color[2]);
        $this->Cell(32, $lineheight, Encoding::fixUTF8($this->lang['date']) . ':', 0, 0, 'L');
        $this->SetTextColor(50, 50, 50);
        $this->SetFont($this->font, '', 9);
        ...
        if (!empty($this->time)) {
            ...
            $this->Cell(
                32,
                $lineheight,
                    Encoding::fixUTF8($this->lang['time']) . ':',
                0,
                0,
                'L'
            );
        ...
        if (!empty($this->due)) {
            ...
            $this->SetFont($this->font, 'B', 9);
            $this->SetTextColor($this->color[0], $this->color[1], $this->color[2]);
            $this->Cell(32, $lineheight, Encoding::fixUTF8($this->lang['due']) . ':', 0, 0, 'L');
            $this->SetTextColor(50, 50, 50);
            $this->SetFont($this->font, '', 9);
        ...
        if (count($this->customHeaders) > 0) {
            foreach ($this->customHeaders as $customHeader) {
                ...
                $this->SetFont($this->font, 'B', 9);
                $this->SetTextColor($this->color[0], $this->color[1], $this->color[2]);
                $this->Cell(32, $lineheight, Encoding::fixUTF8($customHeader['title']) . ':', 0, 0, 'L');
                $this->SetTextColor(50, 50, 50);
                $this->SetFont($this->font, '', 9);
        ...
        if ($this->PageNo() == 1) {
            ...
            if ($this->display_tofrom === true) {
                if ($this->displayToFromHeaders === true) {
                    $this->Cell($width, $lineheight, Encoding::fixUTF8($this->lang['from']), 0, 0, 'L');
                    $this->Cell(0, $lineheight, Encoding::fixUTF8($this->lang['to']), 0, 0, 'L');
                    $this->Ln(7);
                ...
                $this->SetTextColor(50, 50, 50);
                $this->SetFont($this->font, 'B', 10);
                $this->Cell($width, $lineheight, Encoding::fixUTF8($this->from[0] ?? 0), 0, 0, 'L');
                $this->Cell(0, $lineheight, Encoding::fixUTF8($this->to[0] ?? 0), 0, 0, 'L');
                ...
                for ($i = 1, $iMax = max($this->from === null ? 0 : count($this->from), $this->to === null ? 0 : count($this->to)); $i < $iMax; $i++) {
                    ...
                    if (!empty($this->from[$i]) || !empty($this->to[$i])) {
                        $this->Cell($width, $lineheight, Encoding::fixUTF8($this->from[$i] ? '' : $this->from[$i]), 0, 0, 'L');
                        $this->Cell(0, $lineheight, Encoding::fixUTF8($this->to[$i] ? '' : $this->to[$i]), 0, 0, 'L');
                    }
                    $this->Ln(5);
                }
                $this->Ln(-6);
        ...
        if (!isset($this->productsEnded)) {
            ...
            $this->Cell(
                ...
                10,
                    Encoding::fixUTF8($this->lang['product']),
                0,
                0,
                'L',
                0
            ...
            if (isset($this->quantityField)) {
                ...
                $this->Cell(
                    $width_other,
                    10,
                        Encoding::fixUTF8($this->lang['qty']),
                    0,
                    0,
                    'C',
                    0
            ...
            if (isset($this->vatField)) {
                ...
                $this->Cell(
                    $width_other,
                    10,
                        Encoding::fixUTF8($this->lang['vat']),
                    0,
                    0,
                    'C',
                    0
            ...
            if (isset($this->priceField)) {
                ...
                $this->Cell(
                    $width_other,
                    10,
                        Encoding::fixUTF8($this->lang['price']),
                    0,
                    0,
                    'C',
                    0
            ...
            if (isset($this->discountField)) {
                ...
                $this->Cell(
                    $width_other,
                    10,
                        Encoding::fixUTF8($this->lang['discount']),
                    0,
                    0,
                    'C',
                    0
            ...
            if (isset($this->totalField)) {
                ...
                $this->Cell(
                    $width_other,
                    10,
                        Encoding::fixUTF8($this->lang['total']),
                    0,
                    0,
                    'C',
                    0
    ...
    public function Body()
    {
            ...
            foreach ($this->items as $item) {
                ...
                if ($item['description']) {
                    ...
                    $calculateHeight->MultiCell(
                        $this->firstColumnWidth,
                        3,
                            Encoding::fixUTF8($item['description']),
                        0,
                        'L',
                        1
                    );
                ...
                if (isset($this->vatField)) {
                    $this->Cell($this->columnSpacing, $cHeight, '', 0, 0, 'L', 0);
                    if (isset($item['vat'])) {
                        $this->Cell($width_other, $cHeight, Encoding::fixUTF8($item['vat']), 0, 0, 'C', 1);
                    } else {
                        $this->Cell($width_other, $cHeight, '', 0, 0, 'C', 1);
                    }
                }
                if (isset($this->priceField)) {
                    $this->Cell($this->columnSpacing, $cHeight, '', 0, 0, 'L', 0);
                    if (isset($item['price'])) {
                        $this->Cell($width_other, $cHeight, Encoding::fixUTF8($item['price']), 0, 0, 'C', 1);
                    } else {
                        $this->Cell($width_other, $cHeight, '', 0, 0, 'C', 1);
                    }
                }
                if (isset($this->discountField)) {
                    ...
                    if (isset($item['discount'])) {
                        $this->Cell($width_other, $cHeight, Encoding::fixUTF8($item['discount']), 0, 0, 'C', 1);
                    } else {
                        $this->Cell($width_other, $cHeight, '', 0, 0, 'C', 1);
                    }
                }
                if (isset($this->totalField)) {
                    $this->Cell($this->columnSpacing, $cHeight, '', 0, 0, 'L', 0);
                    if (isset($item['total'])) {
                        $this->Cell($width_other, $cHeight, Encoding::fixUTF8($item['total']), 0, 0, 'C', 1);
                    } else {
                        $this->Cell($width_other, $cHeight, '', 0, 0, 'C', 1);
                    }
                }
            ...
            if ($this->totalsAlignment == 'horizontal') {
                ...
                for ($i=0;$i<$totalsCount;$i++) {
                    $this->Cell(
                        ...
                        7,
                            Encoding::fixUTF8($this->totals[$i]['name']),
                        1,
                        0,
                        'C',
                        true
                ...
                for ($y=0;$y<$totalsCount;$y++) {
                    $this->Cell(
                        ...
                        6,
                            Encoding::fixUTF8($this->totals[$y]['value']),
                        'LRB',
                        0,
                        'C',
                ...
                foreach ($this->totals as $total) {
                    ...
                    $this->Cell($width_other, $cellHeight, Encoding::fixUTF8($total['value']), 0, 0, 'C', 1);
                    $this->Ln();
                    $this->Ln($this->columnSpacing);
                }
            }
        ...
        if ($this->badge) {
            ...
            $this->Write(10, Encoding::fixUTF8($badge));
            $this->Rotate(0);
        ...
        foreach ($this->addText as $text) {
            if ($text[0] == 'title') {
                $this->SetFont($this->font, 'b', 9);
                $this->SetTextColor(50, 50, 50);
                $this->Cell(0, 10, Encoding::fixUTF8($text[1]), 0, 0, 'L', 0);
                $this->Ln();
                $this->SetLineWidth(0.3);
                $this->SetDrawColor($this->color[0], $this->color[1], $this->color[2]);
                $this->Line(
            ...
            if ($text[0] == 'paragraph') {
                ...
                $this->SetFont($this->font, '', 8);
                $this->MultiCell(0, 4, Encoding::fixUTF8($text[1]), 0, 'L', 0);
                $this->Ln(4);
            }
        }
    }
    ...
    public function Footer()
    {
        ...
        $this->SetFont($this->font, '', 8);
        $this->SetTextColor(50, 50, 50);
        $this->Cell(0, 10, Encoding::fixUTF8($this->footernote), 0, 0, 'L');
        $this->Cell(
            0,
            10,
                Encoding::fixUTF8($this->lang['page'] . ' ' . $this->PageNo() . ' ' . $this->lang['page_of'] . ' {nb}'),
            0,
            0,
            'R'
        );
